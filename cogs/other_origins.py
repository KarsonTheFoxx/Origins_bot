...
async def autocomplete(inter, string:str):
    origin_names = []
    for packs in listdir("./other_data/"):
        with open(f"./other_data/{packs}/data.json") as packs:
            packs = load(packs)
...
# Cog handler      
class other_origin_cog(commands.Cog):
    def __init__(self,bot):
        self.bot = bot
    
    # Command call
    @commands.slash_command(name="other-origins")
    ...
    async def origin(self, inter, origin:str=commands.Param(autocomplete=autocomplete)):
        ...
        for packs in listdir("./other_data/"):
            pack_name = packs
            with open(f"./other_data/{packs}/data.json", "rb") as packs:
                packs = load(packs)
...
# Setup 
def setup(bot):
    bot.add_cog(other_origin_cog(bot))